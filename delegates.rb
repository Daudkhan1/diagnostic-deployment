...
#
STDERR.puts "CustomDelegate loaded successfully!"
$stdout.sync = true
...
class CustomDelegate
    ...
    def s3source_object_info(options = {})
        STDERR.puts "DEBUG: s3source_object_info function is running!"

        return nil if context.nil? || context['identifier'].nil? || context['identifier'].empty?

        bucket = ENV['CANTALOUPE_S3SOURCE_BASICLOOKUPSTRATEGY_BUCKET_NAME']
        return nil if bucket.nil? || bucket.empty?

        identifier = context['identifier']
        STDERR.puts "DEBUG: Identifier is #{identifier}"

        # Extract filename and type from the identifier, but ignore the original extension
        match = identifier.match(/^(.+?)(?:\.(thumbnail|processed))?\.([^.]+)$/)

        unless match
            STDERR.puts "ERROR: Identifier does not contain a valid extension"
            return nil
        end

        filename = match[1]  # Example: 'Kidney-29476'
        file_type = match[2] || "processed"  # Will be 'thumbnail', 'processed', or default to 'processed'

        STDERR.puts "DEBUG: Parsed values -> Filename: #{filename}, File Type: #{file_type}"

        # Always use the processed folder structure and .tiff extension
        key = "processed/#{filename}/#{file_type}.tiff"
        STDERR.puts "DEBUG: Using key: #{key}"
